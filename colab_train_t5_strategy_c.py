"""\
Strategy C (stage 2): Bidirectional supervised fine-tuning starting from a mixed-language CPT checkpoint

This script intentionally combines:
- Strategy A's *bidirectional supervised fine-tune* dataset construction (mixing EN↔SR rows)
- Strategy C's *mixed SR+EN continued pretraining (CPT)* checkpoint as the starting weights

Only intentional differences vs Strategy A (`colab_train_t5_strategy_a.py`):
- `model_name` is resolved from a CPT output directory (from `colab_pretrain_t5_strategy_c.py`).
- Adds `cpt_checkpoint_dir` (auto or explicit) and a helper to derive the default CPT output dir.

Everything else stays baseline-like:
- Deterministic 70/20/10 split
- Tokenization caching to Drive (keyed by both CSVs + mix config)
- Same metrics: BLEU, chrF++, diacritics
- Same checkpointing + resume guard + results.json schema
...
2) Set CONFIG["cpt_checkpoint_dir"] to your Strategy C CPT output folder (recommended)
   or leave it as "auto" if you used the default pretrainer config.

3) Run:
   !python /content/drive/MyDrive/T5/colab_train_t5_strategy_c.py
"""
...
CONFIG = {
    ...
    # Strategy C CPT checkpoint
    # Option A (recommended): explicit CPT output folder from `colab_pretrain_t5_strategy_c.py`
    "cpt_checkpoint_dir": "auto",

    # Model name (resolved in main)
    "model_name": None,

    # Direction (for consistency with existing scripts)
    "translation_direction": "bidirectional",

    # Strategy A-style mixing config
    "mix": {
...
# ---------------------------
# Helpers (kept in-file, baseline-style)
# ---------------------------
...
def cpt_expected_output_dir() -> Path:
    """Derive the default Strategy C CPT output dir for pretrainer defaults.

    If you changed `colab_pretrain_t5_strategy_c.py` CONFIG, set CONFIG['cpt_checkpoint_dir'] explicitly.
    """
    ...
    # Strategy C pretrainer mix defaults
    sr_frac = 0.5
    en_frac = 0.5
...
# ---------------------------
# Strategy A dataset building (copied)
# ---------------------------
...
def build_bidirectional_dataset(load_dataset_fn):
    ...
    print(f"[Strategy C] Loading: {sr_path}")
    ds_sr = load_dataset_fn("csv", data_files=str(sr_path))["train"]

    print(f"\n[Strategy C] Base sizes: eng_to_sr={len(ds_eng)}, sr_to_eng={len(ds_sr)}")
    ...
    print(
        f"[Strategy C] After mixing: combined={len(combined)} "
        f"(eng_frac={eng_frac}, sr_frac={sr_frac}, mode={sample_mode})"
    )
...
# ---------------------------
# Metrics helpers (baseline copied)
# ---------------------------
...
def main() -> None:
    ...
    # Build combined bidirectional dataset
    dataset = build_bidirectional_dataset(load_dataset)

    # Sanity checks on each direction's targets
    # - eng_to_sr targets are Serbian
    # - sr_to_eng targets are English
    # We'll run diacritics check on the full combined dataset, but it will only
    # be meaningful for the Serbian-target subset.
    print_diacritics_sanity(dataset.filter(lambda x: x["direction"] == "eng_to_sr"), column="target")

    # Fixed split 70/20/10
    valid_ratio = float(CONFIG.get("validation_split", 0.2))
    test_ratio = float(CONFIG.get("test_split", 0.1))
    ...
    def preprocess(examples: dict) -> dict:
        ...
        inputs = [prefix_map[d] + s for d, s in zip(directions, sources)]
    ...
    # include both CSV fingerprints + mix config, like Strategy A
    eng_fp = file_fingerprint(data_path("eng_to_sr.csv"))
    sr_fp = file_fingerprint(data_path("sr_to_eng.csv"))
    mix = CONFIG.get("mix") or {}
    e = float(mix.get("eng_to_sr_fraction", 1.0))
    s = float(mix.get("sr_to_eng_fraction", 1.0))
    sample_mode = str(mix.get("sample_mode", "random"))
    ...
    print("\nTraining (Strategy C translation fine-tune)...")
